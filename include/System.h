...
private:
    ...
    // 为什么LocalMapping 和LoopClosing有对应的std::thread线程变量 而Tracking线程没有呢？
    // 因为Tracking线程是主线程 另外两者为子线程 主线程通过持有两个子线程的指针mptLocalMapping 和mptLoopClosing控制子线程
    // 虽然在编程上实现三大线程构成父子关系 但逻辑上认为三者是并发的 并不存在谁控制谁的问题
